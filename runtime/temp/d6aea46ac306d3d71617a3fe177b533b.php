<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:56:"D:\shop\public/../application/admin\view\index\menu.html";i:1604733577;}*/ ?>
...
<div id="main-div">
    <div id="menu-list">
        <?php if(is_array($menu) || $menu instanceof \think\Collection || $menu instanceof \think\Paginator): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;if($vo['parent_id'] == '0'): ?>
        <ul id="menu-ul">
            <li class="explode" key="02_cat_and_goods" name="menu">
            <?php echo $vo['p_name']; ?>
                <ul>
                    <?php if(is_array($menu) || $menu instanceof \think\Collection || $menu instanceof \think\Paginator): $i = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;if($v['parent_id'] == $vo['id']): ?>
                    <li class="menu-item"><a href="<?php echo url($v['controller_name'] .'/'.$v['action_name']); ?>" target="main-frame"><?php echo $v['p_name']; ?></a></li>
